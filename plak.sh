...
INI
    ...
    if $has_db_config && { $auto_yes || gum confirm "Existing Plak database config found. Use it and skip database setup?"; }; then
        ...
        # If DB_PORT is 3307 or other (Plak's dedicated MariaDB), connect to that
        # specific port without sudo. unix_socket auth maps OS user to MariaDB user.
        if [ "$DB_PORT" = "3306" ]; then
            ...
            if { echo "$sql_command" | mysql -h "$DB_HOST" -P "$DB_PORT" &> /dev/null; } \
        ...
        if $user_created_successfully; then
            ...
            if [ "$DB_PORT" = "3306" ]; then
                ...
                if [ -z "$root_pass" ]; then
                    if { echo "$sql_command" | $SUDO_CMD mysql -u "$root_user" &> /dev/null; } \
                        || { echo "$sql_command" | $SUDO_CMD mysql &> /dev/null; } \
                        || { echo "$sql_command" | mysql &> /dev/null; }; then
                        echo "   - ✅ Manual database user creation successful."
                        user_created_successfully=true
                    fi
                else
                    if { echo "$sql_command" | $SUDO_CMD mysql -u "$root_user" -p"$root_pass" &> /dev/null; } \
                        || { echo "$sql_command" | mysql -u "$root_user" -p"$root_pass" &> /dev/null; }; then
                        echo "   - ✅ Manual database user creation successful."
                        user_created_successfully=true
                    fi
                fi
            else
                # Plak's dedicated MariaDB on custom port
                if [ -z "$root_pass" ]; then
                    if { echo "$sql_command" | mysql -h "$DB_HOST" -P "$DB_PORT" &> /dev/null; } \
                        || { [ -S "$mariadb_socket" ] && echo "$sql_command" | mysql --protocol=SOCKET --socket="$mariadb_socket" &> /dev/null; } \
                        || { echo "$sql_command" | $SUDO_CMD mysql -h "$DB_HOST" -P "$DB_PORT" &> /dev/null; }; then
                    ...
                    if { echo "$sql_command" | mysql -h "$DB_HOST" -P "$DB_PORT" -u "$root_user" -p"$root_pass" &> /dev/null; } \
                        || { [ -S "$mariadb_socket" ] && echo "$sql_command" | mysql --protocol=SOCKET --socket="$mariadb_socket" -u "$root_user" -p"$root_pass" &> /dev/null; }; then
                        echo "   - ✅ Manual database user creation successful."
                        user_created_successfully=true
                    fi
                fi
